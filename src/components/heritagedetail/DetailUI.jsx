...
export const IconHeart = ({ active }) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill={active ? "#6e0000" : "none"} stroke={active ? "#6e0000" : C.textDark} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export const IconExternalLink = () => (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#6a7282" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export const IconChevronLeft = () => (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#6a7282" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
export function SectionTitle({ children }) {
    return (
        <div style={{ borderBottom: `3px solid ${C.yellow}`, paddingBottom: 14, marginBottom: 24 }}>
            <h2 style={{ fontFamily: fontSerif, fontSize: 28, fontWeight: 700, color: C.navy, margin: 0 }}>{children}</h2>
        </div>
    );
}
    ...
    return (
        ...
        >
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                ...
                >
                    {review.userIcon ? <img src={review.userIcon} alt={review.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : review.name[0]}
                </div>
                <span style={{ fontSize: 16, fontWeight: 700, color: C.navy, fontFamily: font }}>{review.name}</span>
            </div>
            <p style={{ fontSize: 15, color: C.textDark, lineHeight: 1.8, margin: 0, fontFamily: font }}>{review.text}</p>
            {review.date && <p style={{ fontSize: 12, color: C.gray, margin: 0, fontFamily: font }}>{review.date}</p>}
        </div>
    );
}
