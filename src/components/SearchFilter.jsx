...
const TYPE_OPTIONS = [
    { value: "0", label: "タイプ (全体)" },
    { value: "11", label: "国宝" },
    { value: "12", label: "宝物" },
    { value: "13", label: "史跡" },
    { value: "16", label: "天然記念物" },
    { value: "17", label: "無形遺産" },
...
function SearchFilter() {
    ...
    const [heritageType, setHeritageType] = useState("0");
                ...
                <div className="w-full bg-white rounded-[32px] lg:rounded-[999px] p-4 lg:p-4 shadow-[0_15px_40px_rgba(0,0,0,0.12)] border border-gray-100 transition-all">
                    <div className="flex flex-col lg:flex-row items-center gap-4 lg:gap-4 px-2 lg:px-2">
                        ...
                        <div className="flex-1 relative w-full">
                            <img src={imgIconSearchNavy} alt="" className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 opacity-40 pointer-events-none" />
                            <input type="text" placeholder="国家遺産を検索（例：景福宮、慶州）" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-14 pr-6 h-12 text-[15px] border-none bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-[#000D57]/10 rounded-[999px] outline-none transition-all" />
                        </div>
                        ...
                        <div className="grid grid-cols-2 lg:flex lg:flex-row items-center gap-3 w-full lg:w-auto">
                            ...
                            <div ref={typeRef} className="relative">
                                ...
                                >
                                    <span className="text-[#000D57]">{TYPE_OPTIONS.find((o) => o.value === heritageType)?.label}</span>
                                    <img src={imgIconChevron} alt="" className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 opacity-40 transition-transform duration-300" style={{ transform: `translateY(-50%) rotate(${isTypeOpen ? 180 : 0}deg)` }} />
                                </button>
                                <AnimatePresence>
                                    {isTypeOpen && (
                                        <motion.div
                            ...
                            <div ref={themeRef} className="relative">
                                <button
                                    ...
                                    className="w-full h-12 pl-6 pr-10 bg-gray-50/50 rounded-[999px] outline-none cursor-pointer text-xs font-bold flex items-center justify-between whitespace-nowrap border border-gray-100 hover:bg-white transition-all"
                                >
                                    <span className="text-[#000D57]">{THEME_OPTIONS.find((o) => o.value === theme)?.label}</span>
                                    <img src={imgIconChevron} alt="" className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 opacity-40 transition-transform duration-300" style={{ transform: `translateY(-50%) rotate(${isThemeOpen ? 180 : 0}deg)` }} />
                                </button>
                                <AnimatePresence>
                                    {isThemeOpen && (
                                        <motion.div
                        ...
                        {/* 검색 버튼 */}
                        <button onClick={handleSearch} className="w-full lg:w-14 h-12 bg-[#6E0000] hover:bg-[#000D57] text-white rounded-[999px] lg:rounded-full transition-all duration-300 flex items-center justify-center shadow-lg hover:shadow-xl group">
