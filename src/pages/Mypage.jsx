...
const LIKE_DATA = [
  { id: 10, img: "https://images.unsplash.com/photo-1559827260-dc66d52bef19?q=80&w=400", name: "石窟庵", region: "慶州" },
];
  ...
  return (
    <div style={{ border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "16px", background: C.white, transition: "all 0.2s", cursor: "pointer" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}><CalendarIcon /><span style={{ fontSize: 12, color: C.gray3 }}>{route.date}</span></div>
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 13, color: C.gray2 }}>{route.region}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}><MapPinIcon /><span style={{ fontWeight: 700, color: C.red, fontSize: 13 }}>{route.spots}か所</span></div>
      </div>
    </div>
  );
}
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "140px 20px", display: "flex", flexDirection: "column", gap: 32 }}>
        ...
        <div style={{ background: C.white, borderRadius: 24, padding: "40px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
            <div style={{ width: 120, height: 120, borderRadius: "50%", border: `4px solid ${C.red}`, overflow: "hidden" }}>
              <img src="https://www.figma.com/api/mcp/asset/0464130a-49a3-4a59-b0d0-3bb1d69fb641" alt="profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </div>
            <div>
              <h1 style={{ fontSize: 28, fontWeight: 900, color: C.navy, margin: "0 0 4px" }}>文化遺産探検家</h1>
              <p style={{ fontSize: 15, color: C.gray3, marginBottom: 16 }}>explorer@topaboda.jp</p>
              <div style={{ display: "flex", gap: 24 }}>
                <div><p style={{ fontSize: 12, color: C.gray4, margin: 0 }}>登録日</p><p style={{ fontWeight: 700 }}>2024.01.15</p></div>
                <div><p style={{ fontSize: 12, color: C.gray4, margin: 0 }}>訪問済み</p><p style={{ fontWeight: 700, color: C.red }}>47 か所</p></div>
              </div>
            </div>
          </div>
          <button onClick={() => navigate("/mypage/edit")} style={{ padding: "12px 24px", borderRadius: 12, border: "none", background: C.red, color: "white", fontWeight: 700, cursor: "pointer", transition: "0.2s" }} onMouseEnter={(e) => e.target.style.transform="translateY(-2px)"} onMouseLeave={(e) => e.target.style.transform="translateY(0)"}>プロフィール編集</button>
        </div>
        ...
        <div style={{ background: C.white, borderRadius: 24, padding: "32px", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 40 }}>
            <div><h2 style={{ fontSize: 22, fontWeight: 800, color: C.navy, margin: "0 0 6px" }}>業績ギャラリー</h2><p style={{ fontSize: 14, color: C.gray2 }}>探検の足跡を記録してください</p></div>
            <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              ...
              <div style={{ textAlign: "right" }}><div style={{ fontSize: 24, fontWeight: 900, color: C.red }}>7 <span style={{ fontSize: 14, color: C.gray4 }}>/ 16</span></div><p style={{ fontSize: 12, color: C.gray3, marginTop: 4 }}>達成済み</p></div>
            </div>
          </div>
